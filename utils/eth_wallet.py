...
class eth_wallet_generator:
    ...
    def logger(self, header:str, header_color, message: str, end="\n"):
        print(f"{header_color}[{header:<7}]{Style.RESET_ALL} | {message}", end=end)

    def generate_additional_entropy(self):
    ...
    def create_eth_wallet(self):
        ...
        combined_entropy = self.generate_additional_entropy()
    ...
    def generate_batch_eth_wallets(self, num_wallets:int=10)-> list:
        ...
        for i in range(num_wallets):
            ...
            self.logger(header=f"{i+1}/{num_wallets}", header_color=Fore.BLUE + Style.BRIGHT, 
                        message=f"Wallet {i+1} created: {address[:6]}***{address[-6:]}", end="\r")
        print("")
    ...
    def export_wallets_to_csv(self, wallets: list, filename: str =None):
        ...
        self.logger("Status", header_color=Fore.CYAN + Style.BRIGHT,
                    message=f"Successfully export wallets {Fore.GREEN + Style.BRIGHT}({filename}.csv){Style.RESET_ALL}, check results folder.")
    ...
    def export_wallets_to_excel(self, wallets: list, filename: str =None):
        ...
        self.logger("Status", header_color=Fore.CYAN + Style.BRIGHT,
                    message=f"Successfully export wallets {Fore.GREEN + Style.BRIGHT}({filename}.xlsx){Style.RESET_ALL}, check results folder.")
